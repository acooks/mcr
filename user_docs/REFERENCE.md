...
1. **Auto-Generated (Default):**
   - If you don't specify `--rule-id` when adding a rule, MCR generates a stable hash-based ID
   - The ID is computed from the input tuple: (interface, multicast group, port)
   - Example: `a3f8c2b57d4e4a1b` (16-character hex string)
   - **Stable across reloads:** The same input configuration always produces the same ID
   - This means rules from config files maintain consistent IDs when the config is reloaded
