...
PACKET_SIZE=1400
PACKET_COUNT=9000000   # 9M packets total (60 seconds at 150k pps)
SEND_RATE=150000       # 150k pps - high rate test
...
echo "=== Baseline Performance Test (150k pps) ==="
echo "Expected: High throughput forwarding"
...
# We expect ~80-90% packet forwarding at 150k pps (kernel drops at high rate)
# Generator sends 9M → MCR-1 sees ~7-8M → MCR-2 sees ~7-8M

# Validate MCR-1: Expect ~80% received (allow kernel drops at high rate)
validate_stat /tmp/mcr1.log 'STATS:Ingress' 'matched' 7000000 'MCR-1 ingress matched (9M)' || VALIDATION_PASSED=1
...
validate_stat /tmp/mcr2.log 'STATS:Ingress' 'matched' 7000000 'MCR-2 ingress matched (9M)' || VALIDATION_PASSED=1
...
if [ $BUFFER_EXHAUSTION -gt 90000 ]; then
    log_error "❌ Unexpected buffer exhaustion at 150k pps: $BUFFER_EXHAUSTION packets (>1%)"
    VALIDATION_PASSED=1
else
...
if [ $VALIDATION_PASSED -eq 0 ]; then
    log_info '✅ All validations passed - high throughput forwarding at 150k pps'
    log_info 'Full logs available at: /tmp/mcr{1,2}.log'
    echo ""
    echo "=== ✅ BASELINE TEST PASSED ==="
    echo "System achieved high throughput packet forwarding at 150k pps"
