...
use crate::{Command, FlowStats, ForwardingRule, OutputDestination, RelayCommand, Response};
...
async fn run_flow_task(
    ...
    println!(
        "Flow task started: Listening on {}:{}",
        rule.input_group, rule.input_port
    );
...
async fn monitoring_task(_shared_flows: SharedFlows, reporting_interval: u64) {
...
pub async fn run(user: String, group: String) -> Result<()> {
    ...
    let args = Args::parse();
    // ... (rest of the original run function logic)
