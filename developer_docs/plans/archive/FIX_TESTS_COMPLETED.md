Complete Plan to Fix Test Issues
  ...
  5. Fix test_max_workers_spawning interface (rule_management.rs)
    - Change from "eth0" to "lo" for test portability
  6. Add level verification to test_cli_log_level_set_global (cli_functional.rs)
    - After setting level, verify the returned level actually changed
  7. Fix test_dynamic_worker_spawn_on_add_rule logic (multi_interface.rs)
  ...
  8. Remove test_minimal_10_packets (test_basic.rs)
    - Redundant with test_single_hop_1000_packets
    - Delete entirely
  9. Consolidate scaling tests (test_scaling.rs)
    - Three tests have identical validation logic
    - Refactor into single parameterized test function
  10. Remove CLI/log_level_control overlap (cli_functional.rs)
  ...
  11. Add edge case tests (test_basic.rs)
    - Single packet forwarding
    - Maximum size packet (jumbo frame if supported)
  12. Add convergence topology test (test_topologies.rs)
    - N:1 fanout (multiple inputs → single output)
    - Validates aggregation behavior
  13. Add rule removal during traffic test (rule_management.rs or new file)
    - Start traffic flow
    - Remove rule mid-stream
    - Verify clean shutdown without crashes
  14. Enable supervisor resilience tests (supervisor_resilience.rs)
    - Update for current API
    - Add to test suite in integration.rs
    - Tests: worker restart, rule resync, exponential backoff
  15. Add concurrent rule modification test (multi_interface.rs or rule_management.rs)
